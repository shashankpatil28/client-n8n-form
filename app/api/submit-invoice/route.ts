...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!webhookUrl) {
      console.error('Missing N8N_INVOICE_WEBHOOK_URL environment variable');
      return NextResponse.json(
        { error: 'Invoice webhook URL not configured' },
        { status: 500 }
      );
    }

    console.log('Forwarding invoice submission to n8n webhook...');
    ...
    if (!response.ok) {
      ...
      console.error('n8n invoice webhook returned error:', response.status, errorMessage);
